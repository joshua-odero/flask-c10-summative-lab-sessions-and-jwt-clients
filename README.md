...
The following are ways you can interact with the Notes Application Backend API which is implemented by an SQLITE engine:
...
- **Update a particular entry in the database**. Specify the **id** of the entry item you want to delete from the database.
```bash
DELETE /entries/{id} 
```

- **Delete a particular entry in the database**. Specify the **id** of the entry item you want to delete from the database.
```bash
DELETE /entries/{id} 
```
- **Authenticate a particular user by logging in**. Specify the username and password of the user as the payload.
